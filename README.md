...
**Download:** [GitHub releases](https://github.com/johytber-sketch/sky2cd/releases) has both standalone Windows executables. The GUI exe (`sky2cd-gui.exe`) is also distributed on Nexus Mods; the CLI (`sky2cd.exe`) is GitHub-only, see [`docs/NEXUS_LISTING.md`](docs/NEXUS_LISTING.md).

**Requirements:** Windows 10/11 (64-bit) for the standalone executables — no separate Python install needed. Blender is required to actually open and work with the preview files this tool produces. CrimsonForge and a real game install are optional, needed only for live donor verification or DMM packaging.

**Honest limitation:** `Sky2Cd` is not a fully automatic Skyrim-to-Crimson-Desert wearable converter. Its primary output is a Blender preview/mockup for artist review, not a fitted, rigged, tested, or game-ready outfit. The artist remains responsible for fit, clipping, materials, native rig/weights, rebuild/export, and in-game/animation testing.

**Quick start:**

```powershell
sky2cd blender-handoff --input outfit.zip --out .\handoff_out
# Open handoff_out\handoff\import_script.py in Blender, then inspect, fit, clip,
...
**Historical assessment (superseded by the "Technical safety notes / geometry history" section below):**
...
identical to a new one and hide any update.

Windows may show a SmartScreen "Windows protected your PC" prompt for the
standalone executables because they are not code-signed; choose "More info" then
"Run anyway" if you trust the source you downloaded them from.

`.nif` input requires a working PyNifly install (see Developer / CLI workflow
below) — without it, `sky2cd convert`/`blender-handoff` raise a clear importer
error instead of failing silently. Optional live donor verification / DMM
packaging requires both `--packages-path` (a real game install) and
`--crimsonforge-home` (a separately installed CrimsonForge copy); omitting either
one keeps the rest of the workflow fully usable, just without that optional step.

For a fuller troubleshooting list (missing Python for a source install, missing
Blender, missing CrimsonForge/game paths, stale GUI processes), see
...
GPL-3.0-licensed [PyNifly](https://github.com/BadDogSkyrim/PyNifly) project;
CrimsonForge (used only for optional live donor verification/DMM packaging) is a
separate, user-installed, MIT-licensed tool that is never vendored or bundled.

Full attribution, exact license texts, and an open compliance item (the exact
PyNifly source revision behind the bundled DLL has not yet been recorded/verified
against GPL-3.0 source-availability requirements) are recorded in
[`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md). Read it before redistributing a
...
- [`docs/NEXUS_LISTING.md`](docs/NEXUS_LISTING.md) — the reference copy for `Sky2Cd`'s Nexus Mods page (GUI-only distribution).
- [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) — third-party licensing and the current public-binary release blocker.
